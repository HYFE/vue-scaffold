<template>
    <div :class="$style.wrap">
        <div :class="$style.content" @click="onClick">
            <i :class="[$style.arrow, { [$style.placeholder]: !hasChilds, [$style.active]: expand}]"></i>
            <label :class="$style.checkbox">
                <input type="checkbox" @change="onCheck($event.target.checked)" ref="checkbox">
                <span :class="$style.checkIcon"></span>
            </label>
            <span :class="$style.text">{{data.value}}</span>
        </div>
        <div :class="[$style.childs, { [$style.show]: expand}]"
            ref="childs"
            v-if="hasChilds"
